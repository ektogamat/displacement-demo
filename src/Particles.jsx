  ...
  const particlesMaterial = useMemo(() => {
    return new THREE.ShaderMaterial({
      ...
      uniforms: {
        size: { value: 3 },
        scale: { value: 1 },
        color: { value: new THREE.Color("#2d6abb").multiplyScalar(5) },
      },
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          float ll = length(xy);
          gl_FragColor = vec4(color, step(ll, 0.25));
        }
      `,
    });
  }, []);
  ...
  useFrame((state, delta) => {
    if (particlesRef.current) {
      ...
      for (let i = 0; i < particlesCount; i++) {
        ...
        positions[index + 0] += noiseValueX * 0.1 * speeds[i];
        positions[index + 1] += noiseValueY * 0.1 * speeds[i];
        positions[index + 2] += noiseValueZ * 0.1 * speeds[i];
      }
